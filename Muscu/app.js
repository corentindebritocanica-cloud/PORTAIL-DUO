...
/* Le coach conversationnel (chat + generation de bilans via Gemini/Groq) a ete
   retire le 23/09/26 -- decision documentee dans PROBLEMES_RESOLUS.md et dans
   ce README, suite a une instabilite jugee ingerable par Corentin apres
   plusieurs incidents dans une meme session (chacun corrige individuellement,
   mais l'accumulation a rendu la fonctionnalite inutilisable en pratique).
   AUCUNE donnee n'a ete supprimee de Firestore : les anciens bilans deja
   generes restent lisibles ci-dessous, la clef API et les conversations
   passees restent en base (juste plus affichees ni utilisees). */
...
function renderCoachBlock(){
  ...
  box.textContent = existing; /* texte deja genere avant le retrait du coach, jamais interprete comme du HTML */
