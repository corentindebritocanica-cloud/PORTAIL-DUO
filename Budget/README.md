...
- ⚠️ **Anomalie repérée au passage, non corrigée** : `manifest.json` référence l'icône en `icone.png` (minuscule), alors que le fichier réel dans le dépôt s'appelle `icone.PNG` (majuscules) — ce qui peut casser l'icône d'installation PWA selon la sensibilité à la casse du serveur. Le nouveau `sw.js`, lui, référence bien `icone.PNG` (le fichier réel), donc n'est pas affecté. À corriger séparément si besoin.
