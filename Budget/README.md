...
- **Déclenchement** : à chaque snapshot de `mois` (`planifierPublicationPortail`), **seulement après un snapshot venu du serveur** (`portailServeurVu`, indépendant de `state.serveurVu` qui ne se met pas à jour en arrière-plan), regroupé 2,5 s. **Depuis le 23/09/2026, republié à chaque snapshot serveur même si le contenu n'a pas changé** (voir README du Portail, section « Résumé pour le Portail » : `maj` doit refléter la dernière ouverture vérifiée, pas le dernier vrai changement). **Flush immédiat au `pagehide`/`visibilitychange`** (même date) : si la page se ferme avant la fin des 2,5 s (aller-retour rapide dans l'app), le `setTimeout` en attente est publié tout de suite au lieu d'être perdu avec la page. Erreurs absorbées. **v2 (23/09/2026, soir)** : regroupement ramené à 0,3 s, connexion à la boîte aux lettres ouverte dès le démarrage, `portailOk` suit l'accusé de réception serveur, et au départ de la page secours par l'API REST de Firestore en `fetch keepalive` (le flush simple était perdu : file d'écriture en mémoire). Détails et tests : README du Portail, « Publication fiable du résumé — v2 ».
